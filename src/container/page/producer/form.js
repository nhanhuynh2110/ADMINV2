...
const LINK = STORELINK.PRODUCERLINK
...
const FormHandle = (props) => {
  ...
  const onSubmit = () => {
    ...
    if (!isAdd) {
      ...
      api.producer.update(dt, (err, resp) => {
        if (err) return alert('update fail')
        history.push(LINK.GRID)
      })
    } else {
      api.producer.insert(formData, (err, resp) => {
  ...
  >
    ...
    <div className='row'>
      <div className='col-md-12'>
        <Field.CheckBox field={isActive} id='producer-active-id' defaultChecked={activeChecked} value={isActive.value} name={isActive.name} text='Active' onChange={onChange} />
        <Field.CheckBox field={isHome} id='producer-home-id' defaultChecked={homeChecked} value={isHome.value} name={isHome.name} text='Home' onChange={onChange} />
      </div>
    </div>
  </Form>
}
...
const FormWrapper = forwardRef((props, ref) => {
  ...
  useEffect(() => {
    const parents = (cb) => {
      api.producer.getParents({}, (err, resp) => {
    ...
    const data = (cb) => {
      api.producer.get({id: params.id}, (err, resp) => {
        if (err) return cb(err)
        return cb(null, resp)
      })
    }
